...
class QuotesSpider(scrapy.Spider):
    ...
    def parse_result(self,response):
        ayear = response.css(
            'select#ctl00_ContentPlaceHolder1_ddl_year > option[selected]::text').extract_first()
        aimplement = response.css(
            'select#ctl00_ContentPlaceHolder1_ddlImpl > option[selected]::text').extract_first()
        adistrict = response.css(
            'select#ctl00_ContentPlaceHolder1_ddlDist > option[selected]::text').extract_first()
        acircle = response.css(
            'select#ctl00_ContentPlaceHolder1_ddlDao > option[selected]::text').extract_first()
